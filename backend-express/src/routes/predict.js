...
router.post('/predict', upload.array('photos', 10), async (req, res) => {
  try {
    ...
    const finalRawText =
      raw_text || description || specific_details || '';

    const finalTrackingId =
      tracking_id || `CP-${Date.now()}`;
    ...
    // Supports detailed flood/drainage information
    // ------------------------------------------------------------
    if (category === 'flood') {
      try {
        let parsedImpact = [];
    ...
    // ------------------------------------------------------------
    // AI image analysis
    // Only first image is sent to the AI model
    // ------------------------------------------------------------
    if (
      firstFile &&
      (category === 'garbage' || category === 'road_damage')
    ) {
      try {
        if (category === 'garbage') {
    ...
    // ------------------------------------------------------------
    // Convert AI model result into severity
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Convert AI model result into priority score
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Cover image
    // First successfully uploaded image
    // ------------------------------------------------------------
    const coverImageUrl =
    ...
    const savedReport = await Report.create({
      ...
      specific_details:
        specific_details || finalRawText,

      // Plain URL instead of JSON.stringify
      ...
      await ReportImage.bulkCreate(
        uploadedImages.map(
          (img, index) => ({
            ...
            // Only first image receives ML analysis
            ml_analysis:
              img.isFirst
                ? mlAnalysisPayload
                : null,
